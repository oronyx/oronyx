...
namespace onx::textmode
{
    namespace
    {
        ...
        static constexpr size_t BYTES_PER_GLYPH = FONT_HEIGHT;
        static constexpr size_t WIDE_BYTES_PER_GLYPH = FONT_HEIGHT * 2;

        inline const uint8_t* get_glyph(uint32_t cp) 
        {
            constexpr size_t fonts_size = sizeof(FONTS);
            size_t offset = cp * BYTES_PER_GLYPH;
            if (offset >= fonts_size)
                return &FONTS['?' * BYTES_PER_GLYPH];
            return &FONTS[offset];
        }

        inline bool is_wide_char(uint32_t cp) 
        {
        ...
        inline size_t get_glyph_width(uint32_t cp) 
        {
        ...
        {
            uint8_t first = static_cast<uint8_t>(*str);
            if ((first & 0x80) == 0) 
            {
                *cp = first;
                return 1;
        ...
        void _putchar(uint32_t cp) 
        {
            ...
            for (size_t y = 0; y < FONT_HEIGHT; ++y) 
            {
                ...
                for (size_t x = 0; x < char_width; ++x) 
                {
                    ...
                    size_t px = x_pos * char_width + x;
                    size_t py = y_pos * FONT_HEIGHT + y;
                    
                    if (px >= width || py >= height) 
                        continue;
                    
                    size_t fb_offset = py * pitch + px;
                    uint8_t current_byte = glyph[y + byte_offset * FONT_HEIGHT];
    ...
    void init(struct limine_framebuffer* framebuffer) 
    {
        if (!framebuffer || !framebuffer->address) 
            return;
        
        fb = framebuffer;
        buffer = (uint32_t*)framebuffer->address;
